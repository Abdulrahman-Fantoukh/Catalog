...
engine = create_engine(
    'sqlite:///itemcatalog.db', poolclass=SingletonThreadPool)
    ...
    if length <= 10:
        ...
        if 'username' not in login_session:
            return render_template('catalog.html', categories=categories,
                                   theList=zip(latestItems, latestItemCat))
        else:
            return render_template('catalog.html', categories=categories,
...
def categoryDetails(category_id):
    ...
    if 'username' not in login_session:
        return render_template('publicCategoryDetails.html',
                               items=items, category=category)
    return render_template('categoryDetails.html', items=items,
...
def newItem():
    ...
    if request.method == 'POST':
        if request.form['title']:
            ...
            session.commit()
            flash(
                'New category Item %s Successfully Created' % (newItem.title))
            return redirect(url_for('categoryDetails',
                                    category_id=category.id,
                                    pictureURL=login_session['picture']))
    else:
        return render_template('newItemForm.html',
                               pictureURL=login_session['picture'])
...
def deleteItem(category_id, item_id):
    if 'username' not in login_session:
        return redirect('/login')
    category = session.query(Category).filter_by(id=category_id).one()
    deletedItem = session.query(Item).filter_by(category_id=category.id,
                                                id=item_id).one()
